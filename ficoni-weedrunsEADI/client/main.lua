	...
	if DoesEntityExist(deliveryPed) then 
        ...
        RequestAnimDict('anim@heists@box_carry@')
			...
			AttachEntityToEntity(
				weedObject, deliveryPed, GetPedBoneIndex(deliveryPed, weedAttach.bone), 
...
AddEventHandler("SellWeedPackage", function()
	...
	while tasking do
		...
		if toolong < 0 then
			tasking = false
			WeedRun = false
		--	TriggerEvent('phone:emailReceived',"Food Delivery","#A-1003","Predugo ti je trebalo da zavrsis skinut si sa dostave.")
		end
	end
		  ...
		  options = {
			  {
				  ...
				  --canInteract = function()
					--isDead = exports["p-ambulancejob"]:GetDeath()
					--return  not IsPedInAnyVehicle(PlayerPedId()) and exports["Inventory"]:hasEnoughOfItem("tacodeliverylist",1,false) and not isDead
				--	end,
				 }
			  },
		  distance = 1.5
...
AddEventHandler("StartSellingWeed", function()
	...
	--	TriggerEvent('phone:emailReceived',"Food Delivery","#A-1003","Prosledio sam ti lokaciju kupca, bit ce ih jos samo se pobrini da im dostavis paket.")
	    salecount = 0	
	    WeedRun = true
		TriggerEvent("SellWeedPackage")
	else 
		TriggerEvent("notification","Vec raznosis",2,2000)	
	end
end)


local predo = false 
local NemaItem = false
RegisterNetEvent("dropoffweed")
AddEventHandler("dropoffweed", function()
	if not predo then 
		predo = true
		local cashPayment = 0
	    local success = true
...
RegisterCommand("weedtest", function()
    TriggerServerEvent('package-delivered')
end)
